...
async function main() {
    ...
    const outcome = await bobNear.sendMoney("near", nearAmount(50));
...
function nearAmount(value: number): BN {
    const amount = utils.format.parseNearAmount(value.toFixed());
    if (amount == null) {
...
main().catch((error) => { console.log("An error occurred", error)}).finally(() => { console.log("Terminated") })
